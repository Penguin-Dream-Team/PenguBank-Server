...
class TOTPAuthenticator() {
    ...
    fun createCredentials(): TOTPCredentials {
        ...
        val key = buffer.copyOf(SECRET_BITS / 8)
    ...
    fun createOneTimePassword(secretKey: TOTPSecretKey, time: Instant): Int {
    ...
    private fun calculateScratchCodes(buffer: ByteArray): List<Int> {
        ...
        while (scratchCodes.size < SCRATCH_CODES) {
            val scratchCodeBuffer = buffer.copyOfRange(
    ...
    private fun calculateScratchCode(scratchCodeBuffer: ByteArray): Int {
        ...
        var scratchCode = 0
        for (i in 0 until BYTES_PER_SCRATCH_CODE) {
